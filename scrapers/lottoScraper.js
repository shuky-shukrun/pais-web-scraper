...
async function updateDB() {
  ...
  const archive =
    "https://www.pais.co.il/lotto/showMoreResults.aspx?fromIndex=1&amount=99";
  const lastLotterySelector =
    "#moreRecordsOL > li:nth-child(1) > " +
    "div.archive_list_block.lotto_number > div:nth-child(3)";

  // navigate to the archive page and grab the most recent item
  await page.goto(archive, { waitUntil: "domcontentloaded" });
  let lastLotteryOnWeb = await page.$eval(lastLotterySelector, (lotteryNum) =>
    parseInt(lotteryNum.textContent)
  );
  ...
  if (lastLotteryOnWeb > lastLotteryInDB._id) {
    log +=
      "Need to scrap " +
      (lastLotteryOnWeb - lastLotteryInDB._id) +
      " lotteries\n\n";
    console.log(
      "Need to scrap " + (lastLotteryOnWeb - lastLotteryInDB._id) + " lotteries"
    );

    let newScraped = await scrapLotteries(
      lastLotteryInDB._id + 1,
      lastLotteryOnWeb,
      page
    );
    for (let i = 0; i < newScraped.length; i++) {
      log += JSON.stringify(newScraped[i], null, 2) + "\n\n";
